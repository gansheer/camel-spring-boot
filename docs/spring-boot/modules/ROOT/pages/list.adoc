...
:base-non-camel-query: {base-query},attributes='manual-doc,camel-spring-boot-name'
...
=== Non-Camel Miscellaneous Starters

[{index-table-format}]
|===
| Extensions | Artifact | Support Level | Since | Description
|===
indexTable::[{cellformats},{requires},module=others,{base-non-camel-query}]